...
/obj/structure/roguemachine/noticeboard/attack_hand(mob/living/carbon/human/user)
	...
	if(user.job in list("Manor Guard","Inquisitor", "Cataphract", "Watch Captain", "Orthodoxist", "Absolver", "Marshal", "Hand", "Viscount")) //why was KC here but not marshal ?
		can_remove = TRUE
	if(user.job in list("Bathmaster","Merchant", "Innkeeper", "Steward", "Archmagus", "City Herald", "Keeper", "Viscount"))
		can_premium = TRUE
	var/contents
