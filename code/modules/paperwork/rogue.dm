...
/obj/item/paper/scroll/grudge
	...
	desc = "A copy you've taken with you. Unfortunately the dampness of the travels have made it unreadable. You can still add new entries, however. It looks bulky enough to act as a mild blunt weapon."
...
/obj/item/paper/inqslip/accusation
	name = "accusation"
	desc = "A writ of religious suspicion, printed on Valorian parchment: one signed not in ink, but blood. Contrary to the name, these writs - while primarly used to request haemological investigations - can also be used to simply catalogue the blood of others. </br>Fold and seal it, it's only proper."
	marquevalue = 4
	sliptype = 0

/obj/item/paper/inqslip/accusation/get_mechanics_examine(mob/user)
    . = ..()
    . += span_info("ACCUSATIONS are used by the Holy Psydonic Inquisition to mail INDEXERS back to Valoria, either for cataloguing or for further haemological faith-testing.")
    ...
    . += span_info("Activate in your hand, once packaged together, to fold the ACCUSATION-INDEXER into a letter. This letter can then be mailed to Valoria through the HERMES.")
...
/obj/item/paper/inqslip/confession
	...
	marquevalue = 6
	desc = "A writ of religious guilt, printed on Valorian parchment: one signed not in ink, but blood. To sign it is to confess your indulgence in whatever sins've been levied your way; whether it is done willingly or not, however, is a completely different question. </br>Fold and seal it, it's only proper."
	sliptype = 2
...
/obj/item/paper/inqslip/confession/get_mechanics_examine(mob/user)
    ...
    . += span_info("Activate in your hand, once signed, to fold the CONFESSION into a letter. This letter can then be mailed to Valoria through the HERMES.")
    . += span_info("Stamping a folded letter with redtallow will increase the amount of MARQUES that're rewarded upon mailage.")
...
/obj/item/paper/inqslip/arrival
	name = "arrival slip"
	desc = "A writ of arrival, printed on Valorian parchment: one signed not in ink, but blood. Intended for one person and one person only. </br>Fold and seal it, it's only proper."
...
/obj/item/paper/inqslip/arrival/get_mechanics_examine(mob/user)
    ...
    . += span_info("Once signed, left-clicking the SLIP will fold it into a letter. This letter can then be mailed to Valoria through the HERMES.")
    . += span_info("Stamping a folded letter with redtallow will increase the amount of MARQUES that're rewarded upon mailage.")
...
/obj/item/paper/scroll/sell_price_changes/New(loc, list/prices, faction_name)
	...
	if(!faction)
		faction = pick("Weocilyn", "Hammerhold", "Grenzelhoft", "Kingsfield")		//add more as time goes, idk
