...
/datum/antagonist/suitor/greet()
	to_chat(owner.current, span_userdanger("I've been sent here with a purpose. I must secure a marriage with the local lord in order to bolster my house's relations with Lyndvhar, by any means necessary."))
...
/datum/antagonist/suitor/roundend_report()
	...
	if(traitorwin)
		...
		to_chat(owner.current, span_redtext("I've failed to secure a marriage into the house of Lyndvhar! My house will be disappointed!"))
